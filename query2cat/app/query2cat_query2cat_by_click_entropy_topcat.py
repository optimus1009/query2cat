...
def process_cateinfo(cateinfo):
    ...
    top_cat_list = []
    for i in range(len(res_list)):
        if res_list[i]['cat_lv'] == "A":
            top_cat_list.append(res_list[i]['id'])
    return top_cat_list
...
def main():
    ...
    for line in sys.stdin:
        ...
        res_str = ""
        for i in range(len(leaf_cate_list)):
            cate = leaf_cate_list[i]
            res_str += cate
            if i < (len(leaf_cate_list)-1):
                res_str += ","
        print '\t'.join([base64_term_query,  res_str])
